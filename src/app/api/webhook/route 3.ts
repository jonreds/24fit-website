  ...
  try {
    ...
    const response = await fetch(`${API_INTERNAL_URL}/api/clients/webhook-register`, {
      ...
      body: JSON.stringify({
        ...
        password: orderData.password || null, // Password from onboarding form
...
export async function POST(request: NextRequest) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (orderType === "daily_pass") {
      ...
      try {
        await sendDailyPassConfirmationEmail(orderData);
      } catch (emailError) {
      ...
      if (dailyPassRecipients.length > 0) {
        try {
          await sendDailyPassAdminNotificationEmail(orderData, dailyPassRecipients);
        } catch (emailError) {
      ...
      if (subscriptionRecipients.length > 0) {
        try {
          await sendAdminNotificationEmail(orderData, subscriptionRecipients);
        } catch (emailError) {
...
async function sendWelcomeEmail(order: OrderData) {
...
async function sendAdminNotificationEmail(order: OrderData, recipients: string[]) {
  const {
    email,
    firstName,
    lastName,
              ...
              <table style="width: 100%; border-collapse: collapse;">
                <tr>
                  <td style="padding: 8px 16px;">
                    ...
                    <span style="color: #10b981; font-size: 28px; font-weight: 800; display: block;">${formatCurrency(amountTotal, currency)}</span>
                  </td>
            ...
            <td style="padding: 24px;">
              ...
              <table style="width: 100%; border-collapse: collapse;">
                ...
                <tr>
                  ...
                  <td style="padding: 8px 0; color: #1a1a1a; font-size: 14px;">${birthDate}</td>
                </tr>
                <tr>
          ...
          <tr>
            <td style="padding: 0 24px 24px;">
              <table role="presentation" style="width: 100%; border-collapse: collapse;">
                <tr>
                  <td align="center" style="padding: 8px;">
                    <a href="https://dashboard.24fit.it" style="display: inline-block; background-color: #FFCF02; color: #1a1a1a; text-decoration: none; padding: 14px 28px; border-radius: 8px; font-size: 14px; font-weight: 700;">
                      Apri Dashboard →
                    </a>
                  </td>
                  <td align="center" style="padding: 8px;">
                    <a href="https://dashboard.stripe.com/payments/${paymentId}" style="display: inline-block; background-color: #635bff; color: #ffffff; text-decoration: none; padding: 14px 28px; border-radius: 8px; font-size: 14px; font-weight: 600;">
                      Vedi su Stripe →
                    </a>
                  </td>
                </tr>
...
async function sendDailyPassConfirmationEmail(order: OrderData) {
...
async function sendDailyPassAdminNotificationEmail(order: OrderData, recipients: string[]) {
  const {
    email,
    firstName,
    lastName,
